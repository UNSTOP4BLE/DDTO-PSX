	...
	for (Note *note = stage.cur_note; note->pos != 0xFFFF; note++)
	{
		...
		else
		{
			...
			if (stage.middlescroll)
		     	middletoggle = note_xmiddle[(note->type & 0x7) ^ stage.note_swap];

		    	else
		    	middletoggle = note_x[(note->type & 0x7) ^ stage.note_swap];
			...
			if (note->type & NOTE_FLAG_SUSTAIN)
			{
				...
				if (note->type & NOTE_FLAG_SUSTAIN_END)
				{
					if (clip < (24 << FIXED_SHIFT))
					{
						...
						note_dst.x = FIXED_DEC(arrowposx,1) + middletoggle - FIXED_DEC(16,1);
						note_dst.y =  y + clip;
